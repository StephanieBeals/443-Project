...
bool turnLeft(); //turns 90 degrees left
bool turnRight(); //turns 90 degrees right
void moveToPt();
...
uint8_t dynIdx=0; //0 = turnLeft, 1=turnRight
...
bool turnLeft(){
    if (!ranOnce[0]){
        ...
        ranOnce[0]=true; //You have to remember in the CALLER to reset the corresponding ranOnce
        dynVar[0]=yaw;
    }
    if (yaw-dynVar[0] < M_PI/2) {
        ROS_INFO("Disp: %f", yaw-dynVar[0]);
        angular = 0.2;
        linear = 0.0;
        return false;
    } else return true;
