...
void spinAround(){//Change to averages
    ...
    if(stepNo<8 && !turn(DEG2RAD(-45),stepNo)){
        ...
        int i;
        float laserArrayTotal=0;
        float laserArrayAverage=0;

        /*for (i=0; i<639; i++) {
            laserArrayTotal = laserVals[i]+laserArrayTotal;
            ROS_INFO("Count: %f", laserArrayTotal);
        }
        laserArrayAverage=laserArrayTotal/639;
        ROS_INFO("Array average: %f", laserArrayAverage);

        laserMem[stepNo] = laserArrayAverage;*/

        //laserMem[stepNo] = (laserVals[318]+laserVals[319]+laserVals[320])/3;
