...
int main(int argc, char **argv)
{
    ...
    while (ros::ok() && secondsElapsed <= 480)
    {
        ...
        switch (state)
        { // 4 min for collision avoidance (random walk w/ memory), 4 min for corner travelling
        case 0:
            navLogic();
            for (int i = 0; i < 10; i++)
            {
...
bool turn(float turnAmt, int ranIdx)
{ // CCW is +ve, turnAmt is between -pi to pi
    OS_INFO("in turn function");
    if (!ranOnce[ranIdx])
    {
    ...
    if (yaw > goal + 0.08 || yaw < goal - 0.08)
    { // Generall the leniency should be angular*2/10
        std::ofstream outputFile("potato.txt");
        ...
        ROS_INFO("yaw: %f, goal, %f", yaw, goal);
        ROS_INFO("yaw: %f, goal, %f", yaw, goal);
        outputFile <<("yaw: %f, goal, %f", yaw, goal) << std::end1;
        outputFile.close();
        if (turnAmt < 0)
        {
            angular = -0.4;
...
{ // Non-blocking timer
    ...
    auto now = std::chrono::system_clock::now();
    if (std::chrono::duration_cast<std::chrono::milliseconds>(now - startPt).count() < limit)
        return false;
    else
        return true;
}
...
float idxToAng(int idx)
{ // Takes the laser index and returns the angle. Can be negative
    idx = idx - 319;
