...
bool spinhalfway() {
    volatile int minIndex = 0;
    bool half_spin_done = false;
    ROS_INFO("turning half way....");
    //ROS_INFO("Step: %d", stepNo);
    if (stepNo < 4 && !turn(DEG2RAD(-45), stepNo)) {
        stepNo++;
        return false; // Return false if the turn is not completed yet
    } else {
        half_spin_done = true; // Set the flag to true if the turn is completed
    }
    
    // Additional logic can be added here if needed

    return half_spin_done; // Return true if the half spin is done successfully
}
